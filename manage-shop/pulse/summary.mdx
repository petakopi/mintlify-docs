...
| **Target** (dashed) | Linearly smoothed cumulative target from 0 on day 1 to your monthly target on the last day. | First available source, in priority: (1) **manual override** typed into the Calculator, (2) **recipe-derived** — `market price × volume` (with the Calculator's suggested price as a fallback for recipes that don't have a market price set), (3) **trailing 3-month average** net revenue. |
| **Breakeven** (flat) | Horizontal line at the revenue point where profit = 0. | Calculator math: `fixed_cost × avg_price / contribution_margin`. Price prefers your recipes' **market prices** (averaged across recipes that have one set) and falls back to the Calculator's suggested price otherwise — same rule as the target line. Only renders when at least one recipe has COGS. |
...
<Steps>
  ...
  <Step title="Recipe-derived (formula)">
    `avg_price × volume`. The price prefers your recipe's **market price** — what you actually charge — and falls back to the Calculator's suggested price (`(avg_cogs + fixed_cost_per_unit) ÷ (1 − margin)`) for any recipe that doesn't have a market price set. Requires at least one recipe with COGS. Setting market prices on your top sellers makes this target match the revenue you'll actually ring up; without them, the target is what the P&L math suggests you *should* charge.
  </Step>
